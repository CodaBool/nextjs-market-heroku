...
export function idFromReqOrCtx(req, context) {
  let cookies
  if (req) {
    console.log('getting cookie from req')
    cookies = parseCookies(req) // this seems to break if variable is not exactly a request
    console.log('cookie =', cookies)
  } else {
    cookies = parseCookies(context)
    // console.log('loading cookies from context', cookies)
  }
  if (cookies) {
